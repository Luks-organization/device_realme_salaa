...
void AlsCorrection::process(sensors_event_t& event) {
    static AreaRgbCaptureResult screenshot = { 0.0, 0.0, 0.0 };

    ALOGV("Raw sensor reading: %.0f", event.light);

    if (event.light > conf.bias) {
        event.light -= conf.bias;
    }
    ...
    float sensor_raw_calibrated = event.light * conf.calib_gain * state.last_agc_gain;
    if (state.force_update
            || ((event.light < state.hyst_min || event.light > state.hyst_max)
        ...
        float sensor_raw_corrected = std::max(event.light - cumulative_correction, 0.0f);

        float agc_gain = conf.sensor_inverse_gain[0];
        if (sensor_raw_corrected > conf.agc_threshold) {
            float gain_estimate = 0;
            if (conf.hbr) {
                gain_estimate = event.data[2] * 1000.0 / sensor_raw_corrected;
            } else {
                gain_estimate = sensor_raw_corrected / event.data[2];
            }
            for (int i = 0; i < 4; i++) {
        ...
        if (cumulative_correction <= event.light * 1.35
                || event.light * conf.calib_gain * agc_gain < 10000.0
                || state.force_update) {
            ...
            event.light = sensor_corrected;
            state.last_corrected_value = sensor_corrected;
            ALOGV("Fully corrected sensor value: %.0f lux", sensor_corrected);
        } else {
            event.light = state.last_corrected_value;
            ALOGV("Reusing cached value: %.0f lux", event.light);
        }
